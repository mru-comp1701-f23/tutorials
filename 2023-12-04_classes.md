...
2. Write code to read the student data from [students.txt](/img/students.txt). Create a `Student` object for each student (each row of the file). Add the `Student` objects to a list.

3. Write a loop that checks each of the `Student` objects and prints the names of students with a GPA higher than 3.9.
